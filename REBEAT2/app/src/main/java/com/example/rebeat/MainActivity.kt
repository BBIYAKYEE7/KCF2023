...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (proximitySensor != null) {
            ...
            Toast.makeText(applicationContext, "No Proximity Sensor Found", Toast.LENGTH_SHORT)
                .show()
        }
    }
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        if (event?.sensor?.type == Sensor.TYPE_PROXIMITY) {
            ...
            if (previousProximity < 5 && currentProximity >= 5) {
                sendSMSMessage()
            }
    ...
    private fun sendSMSMessage() {
        ...
        try {
            ...
            Toast.makeText(applicationContext, "신고 메세지를 전송하였습니다.", Toast.LENGTH_SHORT).show()
        } catch (ex: Exception) {
            Toast.makeText(applicationContext,"Sending SMS failed.",Toast.LENGTH_SHORT).show()
            ex.printStackTrace()
        }
    }
        ...
        when (requestCode) {
            1 -> {
                if ((grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED)) {
                    ...
                    Toast.makeText(applicationContext, "Permission denied to send SMS", Toast.LENGTH_SHORT)
                        .show()
                }
            }
        }
